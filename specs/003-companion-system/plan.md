...
**Branch**: `003-companion-system` | **Date**: 2026-01-25 | **Spec**: [spec.md](./spec.md)
...
- Data entry screens (e.g., symptom logging /health/symptoms): MUST implement companion-integrated design with full-screen Bai Qi character illustration background (same as Health Details Page, per FR-036), glassmorphism input containers (rgba(255,255,255,0.15), blur(35px), border 1.5px solid rgba(255,255,255,0.4), glow), character avatar with dialogue bubble at top-left (top-20, left-aligned, px-5 padding) displaying contextual message like "哪里不舒服吗？别瞒着我。" / "What's bothering you? Don't hide it from me." (i18n support), minimal glassmorphism back button (no title text) in top-left corner, conversational button workflow ("交给他看看" → "保存"), unified textarea input (6-8 rows, border-free styling with subtle bottom border OR background depth variation), paperclip icon (20-24px) at textarea corner for image upload, dynamic emotional placeholder text (rotating 2-3 variations like "哪怕只有一点点不舒服，也要告诉我。"), Polaroid-style image previews (white border 2-3px, shadow, tilted rotation -5deg to 5deg), fixed bottom buttons (equal width, flex-1 wrappers) - creates immersive companion feel during data entry, transforms clinical data entry into intimate sharing experience, maintains visual consistency, enhances emotional connection (FR-037)
