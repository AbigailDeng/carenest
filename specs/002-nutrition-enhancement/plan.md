...
- Q: If user saves a meal from MealDetailScreen while calendar is already visible, should calendar update immediately? → A: Yes, calendar must refresh immediately when meal is saved, even if MealDetailScreen drawer is still open - provides instant visual feedback, ensures users see their saved meal appear on calendar in real-time without needing to navigate away and back.
...
- Q: What visual style should be used for calendar meal markers based on reference designs? → A: Use small circular progress indicators (donut chart style) similar to Macros display in reference images, with color coding for different meal types - visually appealing, compact, professional appearance matching modern food tracking app aesthetics.
- Q: Should UI improvements apply to entire nutrition module or specific screens? → A: Overall UI improvements for entire nutrition module, prioritizing core user flows (ingredient input → meal suggestions → save meal) - ensures consistent design language, better user experience, aligns with reference app aesthetics (card-based layouts, circular progress indicators, clear hierarchy, modern color schemes).
